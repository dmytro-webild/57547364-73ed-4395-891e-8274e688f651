...
(function() {
  ...
  let isActive = false;
  // 'select' | 'text' | 'image' — parent may narrow targeting via
  // webild-set-editor-mode; default keeps the classic click-anything behavior.
  ...
  const isValidElement = (element) => {
    ...
    if (hasBackgroundImage && !hasChildren && !hasTextContent) {
      // Background-image-only blocks are only a meaningful target when the
      // user is explicitly picking images.
  ...
  // Mode-aware target resolution: 'image' → media only, 'text' → text
  // containers only, 'select' (default) → classic behavior.
  ...
  if (!target || !isValidElement(target)) return;
    ...
    if (editorMode !== 'image' && isTextElement(target)) {
